...
class PartnerController extends Controller
{
    ...
    {
        try {
            // Retrieve all users with the role_id of 3 (partners)
            $partners = User::where('role_id', 3)->get();
    
            // Check if partners were found
            if ($partners->isEmpty()) {
                // Handle case where no partners are found
                return view('partners.list')->with('partners');
            }
    
            // Return the view with the partners data
            return view('partners.list', compact('partners'));

        } catch (\Exception $e) {
            // Handle any exceptions or errors during database retrieval
            return view('partners.list')->withErrors('An error occurred while retrieving partners.');
        }
    }
